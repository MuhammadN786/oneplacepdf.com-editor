...
(() => {
  ...
  function drawHandle(ctx, x, y) { ctx.save(); ctx.setLineDash([]); ctx.fillStyle = '#6ea8fe'; ctx.fillRect(x - HANDLE/2, y - HANDLE/2, HANDLE, HANDLE); ctx.restore(); }
  ...
  function hitTestAt(x, y) {
    for (let i=state.stack.length-1; i>=0; i--) {
      ...
      if (a.type === 'line' || a.type === 'arrow') {
        const [p1,p2] = a.points;
        if (dist([x,y],p1) <= HANDLE) return {index:i,page:currentPage,handle:'p1'};
        if (dist([x,y],p2) <= HANDLE) return {index:i,page:currentPage,handle:'p2'};
        if (pointToSegment([x,y], p1, p2) <= HIT_PAD) return {index:i,page:currentPage,handle:'move'};
      } else {
        const r = a.rect;
  ...
  function buildActionFromDrag(p1, p2, preview) {
    ...
    if (state.tool === 'signature') {
      ...
      getImageCached(state.signatureDataURL); // prewarm cache
